...
const { getDatabase, get, run, query, isPostgres } = require('../database');
...
// GET /api/admin/messages - All messages with project/client info (single query)
...
router.post('/reset-password', verifyToken, requireAdmin, async (req, res) => {
  try {
...
// ===== SERVERS (Client ↔ Server, audit refinment) =====

// GET /api/admin/servers - List servers (with client names)
router.get('/servers', verifyToken, requireAdmin, async (req, res) => {
  try {
    await getDatabase();
...
router.post('/servers', verifyToken, requireAdmin, async (req, res) => {
  try {
...
router.put('/servers/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
...
router.delete('/servers/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    await getDatabase();
...
// ===== TEAMS (Team ↔ Employee via team_members bridge) =====

// GET /api/admin/teams - List teams with members
router.get('/teams', verifyToken, requireAdmin, async (req, res) => {
  try {
    await getDatabase();
...
router.post('/teams', verifyToken, requireAdmin, async (req, res) => {
  try {
...
router.put('/teams/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
...
router.delete('/teams/:id', verifyToken, requireAdmin, async (req, res) => {
  try {
    await getDatabase();
