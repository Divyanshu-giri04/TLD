...
const fs = require('fs');
const { getDatabase, query } = require('../database');
...
// (audit 2.7: /uploads was public with no access control)
router.get('/files/:filename', verifyTokenOrApiKey, async (req, res) => {
  try {
    await getDatabase();
    ...
    if (req.user.role === 'client') {
      ...
      // Resolve crew by JWT id first (with fallback)
